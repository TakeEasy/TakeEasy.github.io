        ...
        export default {
            ...
            methods: {
                ...
                check_mobile() {
                    ...
                    this.axios.get(this.f_setting.base_url + '/user/check_telephone/', {params: {telephone: this.mobile}}).then(response => {
                ...
                send_sms() {
                    ...
                    this.axios.get(this.f_setting.base_url + '/user/send_code/', {params: {'telephone': this.mobile}})
                        .then(response => {
                            if (response.data.code) {
                                this.$message({
                                    message: '发送验证码成功',
                ...
                login_password() {
                    if (this.username && this.password) {
                        //发送请求
                        this.axios.post(this.f_setting.base_url + '/user/login/', {
                ...
                code_login() {
                    ...
                    if (this.mobile && this.sms) {
                        //发送请求
                        this.axios.post(this.f_setting.base_url + '/user/sms_login/', {
            ...
            created(){
                this.f_setting=inject("$settings")
            }
        }
    </script>
            ...
            methods: {
                ...
                register() {
                    if (this.mobile && this.sms && this.password) {
                        this.axios.post(this.f_setting.base_url + '/user/register/', {
                            telephone: this.mobile,
                            code: this.sms,
                            password: this.password
                        }).then(response => {
...
- 最后 在Head组件里 修改点东西 让他们可以点击显示 还有登陆后 显示用户名
    ...
    <template>
        <div class="header">
            <div class="slogan">
                <p>呵呵呵教育 | 帮助有志向的年轻人通过努力学习获得体面的工作和生活</p>
            </div>
            <div class="nav">
                <ul class="left-part">
                    <li class="logo">
...
- 手机号短信登录接口
  - 需要添加一个短信登录的序列化类
    ```Python
    class SmsLoginModelSerializer(serializers.ModelSerializer):
        ...
        def _get_user(self, attrs):
            ...
            code = attrs.get('code')
            cache_code = cache.get(f'sms_code_cache_{telephone}')
            if code == cache_code:
                user = models.User.objects.filter(phone=telephone).first()
                # 用户存在签发token
                if user:
                    # 把使用过的验证码从cache里删除
                    cache.set(f'sms_code_cache_{telephone}', '')
                    return user
                else:
                    raise ValidationError('用户不存在')
            else:
                raise ValidationError('验证码不对或者手机号不合法')
  ...
  - 同样 需要先添加一个注册的序列化类
    ...
    class UserRegisterSerializer(serializers.ModelSerializer):
        ...
        class Meta:
            ...
            extra_kwargs = {
                'password': {'max_length': 18, 'write_only': True},
...
- 通过@action去配置路由哦 最后的view是下面这样
    ...
    class LoginViewSet(ViewSet):
        ...
        def login(self, request, *args, **kwargs):
            user_ser = ser.UserModelSerializer(data=request.data, context={})
            if user_ser.is_valid():
                token = user_ser.context['token']
                username = user_ser.context['user'].username
        ...
        def sms_login(self, request, *args, **kwargs):
            sms_user_ser = ser.SmsLoginModelSerializer(data=request.data, context={})
            if sms_user_ser.is_valid():
                token = sms_user_ser.context['token']
                username = sms_user_ser.context['user'].username

                return LuffyResponse(code=2000, msg='登录成功', token=token, username=username)
            else:
                return LuffyResponse(code=5000, msg=sms_user_ser.errors)
    ...
    class RegisterView(GenericViewSet, CreateModelMixin):
        ...
        serializer_class = ser.UserRegisterSerializer

        def create(self, request, *args, **kwargs):
            response = super().create(request, *args, **kwargs)
            username = response.data.get('username')
            return LuffyResponse(code=1, msg='注册成功', username=username)

    ```
- 最后的user下 urls.py 像下面这样
    ```Python
    from django.urls import path, re_path, include
    from rest_framework.routers import SimpleRouter
    from . import views
...
id = '6a7c4636-6839-4de2-954a-d5385b58233f'
